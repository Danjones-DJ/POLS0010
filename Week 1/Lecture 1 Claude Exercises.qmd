...
# Create correlated predictors (some with multicollinearity!)
income <- rnorm(n, 50000, 15000)
education <- 0.7 * income + rnorm(n, 0, 10000)  # Highly correlated with income!
green_space <- rnorm(n, 30, 10)  # FIX: Changed from (0.5, 0.2) to (30, 10)
...
# Generate wellbeing (the true model includes some but not all variables)
wellbeing <- 30 + 
  0.0003 * income + 
  0.5 * green_space + 
  -0.2 * crime_rate + 
  3 * has_community_centre +
  5 * diversity +
  rnorm(n, 0, 5)
